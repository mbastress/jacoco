 ...
 * @since 0.7.7
 */
@Mojo(name = "report-aggregate-reactor", threadSafe = true)
public class ReportAggregateReactorMojo extends AbstractReportMojo {
	...
	public boolean canGenerateReport() {
		return super.canGenerateReport() && !shouldDelayExecution();
	}

	private boolean shouldDelayExecution() {
		return !isLastProjectInReactor();
	}
	...
	private boolean isLastProjectInReactor() {
		return getLastProjectInReactor().equals( project );
	}

	private MavenProject getLastProjectInReactor() {
	...
	void createReport(final IReportGroupVisitor visitor,
		...
		for (final MavenProject project : reactorProjects) {
		    if (!project.equals(getLastProjectInReactor())) {
